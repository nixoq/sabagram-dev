...
export function HomeFeed({ initialPosts, currentUser, isPublicView = false }: HomeFeedProps) {
      ...
      {posts.map((post) => (
        <div key={post.id}>
          {isPublicView ? (
            ...
            <PostCard post={post} currentUserId={currentUser?.id} onLikeUpdate={handleLikeUpdate} />
          )}
        </div>
      ))}
    </div>
